...
class HBNBCommand(cmd.Cmd):
	...
	def do_show(self, line):
		...
		pattern = re.compile(r'^[\da-fA-F]{8}-[\da-fA-F]{4}-[\da-fA-F]{4}-[\da-fA-F]{4}-[\da-fA-F]{12}$')
		pattern1 = re.compile(r'^[A-za-z]+$')
		if line != "":
			args = line.split()
			if (args and len(args) == 1):
				idMatch = re.match(pattern, args[0])
		...
		if os.path.exists("file.json"):
			...
			if (line != ""):
				...
				for key, value in json_str.items():
					...
					if id_value == args[0]:
						obj = BaseModel(**value)
						print(f"[{obj}]") #formatted output still missing double quote
			elif (line == ""):
				for key, value in json_str.items():
					obj = BaseModel(**value)
					print(f"[{obj}]") #formatted output still missing double quote
